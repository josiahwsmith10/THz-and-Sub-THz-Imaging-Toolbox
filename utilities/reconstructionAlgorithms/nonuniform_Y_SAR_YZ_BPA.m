% nonuniform_Y_SAR_YZ_BPA is a reconstructor class that performs a
% 3-D Backprojection Algorithm image reconstruction. The synthetic
% aperture must span the y axis x=0 and can be nonuniformly spaced or use
% a MIMO array, even overlapping. The target can be a 1-D or 2-D target in
% y-z space
%
...
classdef nonuniform_Y_SAR_YZ_BPA < handle
    properties
    ...
    methods
        function obj = nonuniform_Y_SAR_YZ_BPA(im)
        ...
        function getParameters(obj)
            ...
            [X,Y,Z] = ndgrid(0,obj.im.y_m(:),obj.im.z_m(:));
            obj.sizeTarget = [obj.im.numY,obj.im.numZ,1];
        ...
        function verifyReconstruction(obj)
            ...
            if obj.scanner.method ~= "Linear"
                showErrorMessage(obj.im,"Must use 1-D Y SAR scan to use1-D SAR 2-D BPA image reconstruction method!","2D BPA Error");
                obj.isFail = true;
                return
            end
        end
